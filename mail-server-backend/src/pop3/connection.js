...
class POP3Connection extends EventEmitter {
        ...
        if (typeof this._nextHandler === 'function') {
            ...
            return handler(line, err => {
                if (err) {
                    ...
                    // Reset socket timeout after successful continue data processing
                    this._socket.setTimeout(this._server.options.socketTimeout || SOCKET_TIMEOUT, () => this._onTimeout());
                    this.processQueue();
                }
            });
        ...
        if (typeof this['command_' + command] === 'function') {
            this['command_' + command](args, err => {
                if (err) {
                    ...
                    // Reset socket timeout after successful command processing
                    this._socket.setTimeout(this._server.options.socketTimeout || SOCKET_TIMEOUT, () => this._onTimeout());
                    this.processQueue();
                }
            });
    ...
    command_RETR(args, next) {
        ...
        this._server.onFetchMessage(message, this.session, (err, stream) => {
            ...
            stream.once('end', () => {
                ...
                // update counters
                stream.options.ttlcounter(stream.options.key, stream.bytes, stream.options.maxBytes, false, () => {
                    this._server.loggelf({
                        ...
                        _mailbox: message.mailbox,
                        _message_size: message.size,
                        _message_transferred_size: stream?.bytes,
                        _username: this?.session?.user?.username,
                        _user: this?.session?.user?.id,
                        _sess: this.id,
                        _ip: this.remoteAddress
    ...
    command_TOP(args, next) {
        ...
        this._server.onFetchMessage(message, this.session, (err, stream) => {
                    ...
                    if (!finished) {
                        ...
                        if (!headers) {
                            ...
                            if ((match = data.match(/\r?\n\r?\n/))) {
                                ...
                                const buf = Buffer.from(headers.replace(/^\./gm, '..'), 'binary');
                                totalSentBytes += buf.length;
                                this.write(buf);
                            }
                        }
                        if (headers) {
                            ...
                            while (!finished && (match = data.match(/\r?\n/))) {
                                ...
                                const buf = Buffer.from(line.replace(/^\./gm, '..'), 'binary');
                                totalSentBytes += buf.length;
                                this.write(buf);
                                if (linesSent >= lines) {
                                    finished = true;
            ...
            stream.once('error', err => next(err));
            stream.once('end', () => {
                stream.options.ttlcounter(stream.options.key, totalSentBytes, stream.options.maxBytes, false, () => {
                    this._server.loggelf({
                        short_message: '[POP3TOP] OK',
                        _mail_action: 'pop3_top',
                        _message_id: message.id,
                        _message_uid: message.uid,
                        _mailbox: message.mailbox,
                        _message_size: message.size,
                        _message_transferred_size: stream?.bytes,
                        _total_sent_bytes: totalSentBytes,
                        _username: this?.session?.user?.username,
                        _user: this?.session?.user?.id,
                        _sess: this.id,
                        _ip: this.remoteAddress
