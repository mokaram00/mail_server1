...
async function getCategories() {
  try {
    ...
    if (!res.ok) {
      console.error('Categories API failed:', res.status, res.statusText);
      return { categories: [] };
    }
...
export default function ProductsPage() {
  ...
  const handleSearchInputChange = (value: string) => {
    ...
    const timer = setTimeout(async () => {
      if (value.length > 2) {
        try {
          const res = await fetch(`/api/products/search-suggestions?q=${encodeURIComponent(value)}`);
          if (res.ok) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
              {products.map((product: any) => (
                <div key={product._id} className="group relative bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:rotate-1 border border-white/50 overflow-hidden">
                  <Link href={`/products/${encodeURIComponent(product.name.replace(/\s+/g, '-').toLowerCase())}`} className="block">
